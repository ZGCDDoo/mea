...
from auxiliary_fn.auxiliary_fn import auxiliary_fn as aux_fn
from auxiliary_fn.auxiliary_fn import green
from auxiliary_fn.auxiliary_fn import periodize_class
from auxiliary_fn.auxiliary_fn import periodize
from auxiliary_fn.auxiliary_fn import sigma_dc
...
gf_aux = aux_fn.GFAux(fin_sE_to=os.path.join(cwd, "self_moy.dat"), mu=mu)
...
for (i, (sEvec_cw, w_vec)) in enumerate(zip(sEvec_cw_list, w_vec_list)):
    ...
    SDC = sigma_dc.SigmaDC(w_vec, sEvec_cw, beta=beta, mu=mu)
    SDC.calc_sigmadc()