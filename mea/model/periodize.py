...
class Model:
    ...
    def hopping_test(self, kx: float, ky: float):
        ...
        K_sites = np.array([[0.0, 0.0], [np.pi, 0.0], [
                           np.pi, np.pi], [0.0, np.pi]])
        t_array = np.zeros((N_c, N_c), dtype=complex)

        for i in range(N_c):
            for j in range(N_c):
                for K in K_sites:
                    t_array[i, j] += 1 / N_c * np.exp(1.0j * np.dot(
                        K + k, r_sites[i] - r_sites[j])) * self.eps_0(*(K + k))
    ...
    def periodize_Akw_cum(self, kx: float, ky: float, ii: int) -> float:
        ...
        tmp -= -2.0 * self.t * (np.cos(kx) + np.cos(ky)) - \
