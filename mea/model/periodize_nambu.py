...
class ModelNambu:
    ...
    def t_value(self, kx: float, ky: float):  # This is t_ij(k_tilde)
        ...
        t_value_up[0, 0] += 0.0
        t_value_up[0, 1] += -t * ex
        t_value_up[0, 2] += -tp * ex * ey
        t_value_up[0, 3] += -t * ey
        t_value_up[1, 0] += -t * emx
        t_value_up[1, 1] += 0.0
        t_value_up[1, 2] += -t * ey
        t_value_up[1, 3] += -tp * (emx + ey)
        t_value_up[2, 0] += -tp * emx * emy
        t_value_up[2, 1] += -t * emy
        t_value_up[2, 2] += 0.0
        t_value_up[2, 3] += -t * emx
        t_value_up[3, 0] += -t * emy
        t_value_up[3, 1] += -tp * (ex + emy)
        t_value_up[3, 2] += -t * ex
        t_value_up[3, 3] += 0.0
    ...
    def build_gf_ktilde(self, kx: float, ky: float, ii: int):
        ...
        gf_ktilde[0, 0] = gf_ktilde[1, 1] = gf_ktilde[2,
                                                      2] = gf_ktilde[3, 3] = (zz + mu)
        gf_ktilde[4, 4] = gf_ktilde[5, 5] = gf_ktilde[6,
                                                      6] = gf_ktilde[7, 7] = -np.conjugate(-np.conjugate(zz) + mu)
    ...
    def build_cumulants(self):
        ...
        for ii in range(cumulants.shape[0]):
            ...
            tmp[4, 4] = tmp[5, 5] = tmp[6, 6] = tmp[7, 7] = - \
                np.conjugate(-np.conjugate(zz) + mu)
            tmp -= sE
    ...
    def dos_with_trace(self, fout_name="dos_trace_nambu.dat"):
        ...
        for n in range(len_sEvec_c):
            ...
            dos[n] = (2.0 * np.pi)**(-2.0) * dblquad(self.Akw_trace, -
                                                     np.pi, np.pi, self.Y1Limit, self.Y2Limit, args=(n,))[0]
    ...
    def stiffness(self, kx: float, ky: float, ii: int) -> float:
        """ """
        nambu_periodized = self.periodize(kx, ky, self.build_gf_ktilde(
            kx, ky, ii))  # self.periodize_nambu(kx, ky, ii)
    ...
    def periodize_cumulant(self, kx: float, ky: float, ii: int):  # cumulant periodization
        ...
        eps = -2.0 * self.t * (np.cos(kx) + np.cos(ky)) - \
            2.0 * self.tp * np.cos(kx + ky)
        tmp[0, 0] -= eps
        tmp[1, 1] += eps
    ...
    def stiffness_cum(self, kx: float, ky: float, ii: int) -> float:
        ...
        nambu_periodized = self.periodize_cumulant(
            kx, ky, ii)  # linalg.inv(tmp.copy())
        #coskx: float = np.cos(kx)
        #cosky: float = np.cos(ky)
        # tperp = -(coskx - cosky)*(coskx - cosky) # t_perp = -1.0
        #tperp_squared = tperp*tperp
    ...
    def stiffness_trace(self, kx: float, ky: float, ii: int) -> float:
        """4/N_c Trace(F F^Dag) """
        gf_ktilde = self.build_gf_ktilde(kx, ky, ii)
        FF = gf_ktilde[0, 5]**2.0
        tperp_squared = 2.0
        return (tperp_squared * 4.0 * np.real(FF))
    ...
    def matsubara_surface(self, fout: str ="matsubara_surface.dat"):
        ...
        for (i, kx) in enumerate(kxarr):
            for(j, ky) in enumerate(kyarr):
                ...
                Akz_vec[idx, 1] += ky
                Akz_vec[idx,
                        2] += np.absolute(self.periodize_nambu(kx, ky, idx)[0, 1])
                idx += 1
