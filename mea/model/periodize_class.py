...
spec = [
    ('t', double),               # a simple scalar field
    ('tp', double),
    ('mu', double),
    ('w_vec', complex128[:]),
    ('sEvec_c', complex128[:,:])          # an array field
    ]
...
class Model:
    """ """
    def __init__(self, t: float, tp: float, mu: float, w_vec, sEvec_c) -> None:
        """ """

        assert w_vec.shape[0] == sEvec_c.shape[0]
    ...
    def build_gf_ktilde(self, kx: float, ky: float, ii: int):
        """ """
        w = self.w_vec[ii] ; sE = self.sEvec_c[ii] ; mu: float = self.mu
    ...
    def calc_dos(self, fout_name="dos.txt"):
        """ """
        sEvec_c = self.sEvec_c; w_vec = self.w_vec; t = self.t ; tp = self.tp; mu = self.mu
    ...
    def periodize_Gkw_vec(self, kx: float, ky: float): # periodize the green function for all frequencies for one k
        """ """
    ...
    def build_sE_lattice_vec(self, kx: float, ky: float):
        ...
        gf_lattice_vec = self.periodize_Gkw_vec(kx, ky)
        sE_lattice_vec = np.copy(gf_lattice_vec)

        for (i, gf) in enumerate(gf_lattice_vec):
            sE_lattice_vec[i] = (w_vec[i] + mu) - self.eps_0(kx, ky) - 1.0/gf
        ...
        for(itt, (ww, sE)) in enumerate(zip(w_range, sE_range)):
            ...
            for (i, kx) in enumerate(kxarr):
                for(j, ky) in enumerate(kyarr):
                    Akw_vec[idx, 0] += kx
                    Akw_vec[idx, 1] += ky
                    Akw_vec[idx, 2] += self.periodize_Akw(kx, ky, t, tp, sE, ww, mu)
                    idx+=1
