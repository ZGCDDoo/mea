...
sEvec = sEvec_to[:, 1] + 1.0j * sEvec_to[:, 2]
hybvec = hybvec_to[:, 1] + 1.0j * hybvec_to[:, 2]
...
for(ii, (w_vec, gfvec_aux_w)) in enumerate(zip(ac_aux.w_vec_list, gfvec_aux_w_list)):
    ...
    assert hybvec_w.shape == gfvec_aux_w.shape, "Ayaya, hybvec_w and gfvec_aux_w are not same shape..., forgot the grid?"
